...
import {ORDER,ORDERSDISPLAY,ADDEMPLOYEE,GETEMPLOYEE,DELETEEMPLOYEE,EDITEMPLOYEE} from "../reducer/employee-reducer";
import {ADDTOCARD,DELETETOCARD} from '../reducer/addtocard-reducer';
import {GALLERY,GALLERYDATA} from '../reducer/gallery-reducer';
...
export const orders = (employeeData) => {
    debugger;
    return (dispatch) => {
        axios.post('http://localhost:8010/order',employeeData).then((result) => {
            dispatch({
                type: ORDER,
                payload: result.data
            })
...
export const ordersDisplay = () => {
    debugger;
    return (dispatch) => {
        axios.get('http://localhost:8010/orderDisplay').then((result) => {
            dispatch({
                type: ORDERSDISPLAY,
                payload: result.data
            })
...
export const gallerys = (info,jsonObject) => {
    debugger;
    return (dispatch) => {
        axios.post('http://localhost:8010/gallery', info).then((data) => {
            dispatch({
                type: GALLERY,
                payload: jsonObject
            })
...
export const galleryData = () => {
    debugger;
    return (dispatch) => {
        axios.get('http://localhost:8010/gallery').then((result) => {
            dispatch({
                type: GALLERYDATA,
                payload: result.data
            })
