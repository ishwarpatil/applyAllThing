...
con.connect(function (err) {
    ...
    app.post('/gallery', (req, res) => {
        let Arr=[];
        let sampleFile=req.files.galleryPhoto;
        let title=req.body.title;
        let author=req.body.author;
        for(let i=0;i<sampleFile.length;i++){
            sampleFile[i].mv(__dirname+'/Gallery/'+sampleFile[i].name);
            Arr.push(sampleFile[i].name);
        }
        for(let i=0;i<Arr.length;i++){
            let sql = "INSERT INTO gallery (fileName,title,author) VALUES ('" + Arr[i] + "','" + title + "','" + author + "')";
            con.query(sql, (err, result) => {
            });
        }
    });
